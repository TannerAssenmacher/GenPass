...
const SettingsControl: React.FC<SettingsControlProps> = ({ settings, setSettings }) => {
  ...
  return (
    <div className="space-y-6 bg-slate-800/30 p-6 rounded-xl border border-slate-700/30">
      <div className="space-y-2">
        <div className="flex justify-between items-center text-sm font-medium">
          <span className="text-slate-400">Password Length</span>
          <span className="text-orange-400 mono font-bold text-lg">{settings.length}</span>
        </div>
        <input 
          type="range" 
          min="8" 
      ...
      <div className="pt-2">
        ...
        >
          <div className="flex items-center space-x-3">
            <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${settings.funToType ? 'bg-orange-500 text-white' : 'bg-slate-700'}`}>
              <i className="fas fa-brain"></i>
            </div>
            <div className="text-left">
              <span className="block text-sm font-bold">Memorable</span>
              <span className="block text-[10px] opacity-60">Easy-to-type word patterns</span>
            </div>
          </div>
          <div className={`w-6 h-6 rounded-full flex items-center justify-center transition-colors ${settings.funToType ? 'bg-orange-500' : 'bg-slate-700'}`}>
            {settings.funToType && <i className="fas fa-check text-xs text-white"></i>}
          </div>
        </button>
      </div>
    </div>
...
const Checkbox: React.FC<{ label: string, checked: boolean, onChange: () => void }> = ({ label, checked, onChange }) => (
  ...
  >
    ...
    <div className={`w-5 h-5 rounded flex items-center justify-center transition-colors ${checked ? 'bg-orange-500' : 'bg-slate-700'}`}>
